...
*/
let faqToggleButtons = document.getElementsByClassName("toggler");
let faqAnswerParagraphs = document.getElementsByClassName("answer");

for (let i = 0; i < faqToggleButtons.length; i++) {
    faqToggleButtons[i].addEventListener("click", function() {

        // On-click, loop through all FAQ answer fields
        for (let j = 0; j < faqAnswerParagraphs.length; j++) {
            if (j == i) {
                // For the current area, toggle the visibility
                faqAnswerParagraphs[j].classList.toggle("hide");
            } else {
                // For all other areas, hide them (i.e., only one answer is ever visible)
                faqAnswerParagraphs[j].classList.add("hide");
            } 
        }
    });
}
...
let dynamicMessage = document.getElementById("dynamicMessage");
...
function loadQuestionAndAnswers(questionNumber) {
    ...
    if(questionsAndAnswers[questionNumber]["answers"].length == 0){
        ...
        // Add styling to drop-down
        newDropdown.classList.add("select");
...
function submitResponses() {
    ...
    dynamicMessage.innerHTML = "You've got our suggestion. The rest is up to you &#128526;.";
...
function outputSedera() {
    ...
    sederaParagraph.classList.add("padding");
...
function output1099Workers() {
    ...
    // Add styling to paragraph
    insuranceParagraph.classList.add("padding");
...
function outputAddons() {
    ...
    const addOnHeader = document.createElement("h3");

    // Assign text-value to header
    const headerContent = document.createTextNode("Add-Ons");
    addOnHeader.appendChild(headerContent);

    // Create paragraph element
    const addOnParagraph = document.createElement("p");

    // Assign text-value to paragraph
    const paragraphContent = document.createTextNode("Whether you choose to go with the Health Insurance or Medical Cost Sharing option, a great add-on is a Hospital Indemnity policy that will pay out to you in the event that you go to a hospital. These are great to offset the cost of co-pays and deductibles.");

    addOnParagraph.appendChild(paragraphContent);
    addOnParagraph.appendChild(document.createElement("br"));
    addOnParagraph.appendChild(document.createElement("br"));
    addOnParagraph.appendChild(document.createTextNode("Note that we also have other products including Disability Insurance and Life Insurance."));

    // Add styling to paragraph
    addOnParagraph.classList.add("padding");

    // Add header to DOM
    outputDiv.appendChild(addOnHeader);
...
function outputWarnings() {
    if (userWarnings.length > 0) {
        ...
        for (let i = 0; i < userWarnings.length; i++) {
            ...
            // Make output text small
            warningParagraph.classList.add("small");
            warningParagraph.classList.add("orange-text");
